...
subDir = "ATACseq_around_HSF1_Chipseq_motif_normedbyDeseq/"
...
#maxquant<-10
col_fun = colorRamp2(c(0, maxquant), c("white", "brown"))


#draw the matrixes
minabsolute=0
#if you don't know what to set maxabsolute at use 0 and it will autoset but it will not be the same for all samples 
maxabsolute=14
...
mat0pos <- matlist[[1]]
if (maxabsolute==0){
  p<-EnrichedHeatmap(mat0pos, col = col_fun, use_raster = TRUE, name=paste(samplename, " ATAC", sep=""), column_title = paste(samplename," +"))
}else{
  p<-EnrichedHeatmap(mat0pos, col = col_fun, use_raster = TRUE, name=paste(samplename, "ATAC", sep=""), column_title = paste(samplename," +"), top_annotation = HeatmapAnnotation(enriched = anno_enriched(ylim = c(minabsolute, maxabsolute),gp = gpar(col =colorlist[1]))))}
ht_list <- p
...
for (i in 2:length(atacsamplenames)){
  ...
  if (maxabsolute==0){
    ...
    p<-EnrichedHeatmap(mat0pos, col = col_fun, use_raster = TRUE, name=paste(samplename, " ATAC", sep=""), column_title = paste(samplename," +"), top_annotation = HeatmapAnnotation(enriched = anno_enriched(ylim = c(minabsolute, maxabsolute),gp = gpar(col =colorlist[i]))))}
  ht_list <- ht_list+p}
#---
...
draw(ht_list)
#
png(paste(outdir,"EnrichedHeatmap_of_ATAC_overHSFwithchip.png", sep=""), width = 1000, height = 500)
draw(ht_list)
dev.off()
...
col_fun2 <-colorRamp2(c(-30, 0, 30), c("orange", "white", "black"))

mindiff=0
#if you don't know what to set maxdiff at use 0 and it will autoset but it will not be the same for all samples diffs
maxdiff=4
if (maxdiff==0){
  T21diff <- EnrichedHeatmap(T21diffmatrix, name="diff T21", col = col_fun2,  column_title = "diff T21")
  D21diff <- EnrichedHeatmap(D21diffmatrix, name="diff D21", col = col_fun2, column_title = "diff D21")
}else{
  T21diff <- EnrichedHeatmap(T21diffmatrix, name="diff T21", col = col_fun2,  column_title = "diff T21", top_annotation = HeatmapAnnotation(enriched = anno_enriched(ylim = c(mindiff, maxdiff),gp = gpar(col =colorlist[4]))))
  D21diff <- EnrichedHeatmap(D21diffmatrix, name="diff D21", col = col_fun2, column_title = "diff D21", top_annotation = HeatmapAnnotation(enriched = anno_enriched(ylim = c(mindiff, maxdiff),gp = gpar(col =colorlist[2]))))}

ht_list_wtih_diffs = D21diff+T21diff
draw(ht_list_wtih_diffs)

svg(paste(outdir,"EnrichedHeatmap_of_ATAC_overHSFwithchip_diff.svg", sep=""), width = 1000, height = 500)
draw(ht_list_wtih_diffs)
dev.off()

#plot the row means for all positions in the matrix
results=lapply(matlist, FUN=colMeans)
colmeansdf=as.data.frame(do.call(rbind, results))
colmeansdf=t(colmeansdf)
colnames(colmeansdf) <-atacsamplenames
...
#I am here make these the right color
#http://www.sthda.com/english/wiki/ggplot2-line-types-how-to-change-line-types-of-a-graph-in-r-software
#scale_linetype_manual


ggplot(data=meandflong, aes(x=position, y=depth,colour=interaction(genotype, tempature), linetype=interaction(genotype, tempature))) +
  geom_line()+scale_linetype_manual(values=c("solid","solid", "dashed","dashed"))+scale_color_manual(values=c('#006400', '#4B0082','#32CD32', '#DA70D6'))+theme(text = element_text(size=20))+ theme_minimal()



#png(paste(outdir,"Rowmeans_of_ATAC_overHSFwithchip.png", sep=""))
#ggplot(data=meandflong, aes(x=position, y=depth,colour=interaction(genotype, tempature), linetype=interaction(genotype, tempature))) +
#  geom_line(size=2)+scale_linetype_manual(values=c("solid","solid", "dashed","dashed"))+scale_color_manual(values=c('#006400', '#4B0082','#32CD32', '#DA70D6'))+theme(text = element_text(size=20))+ theme_minimal()+ theme(text = element_text(size = 20))
#dev.off()

svg(paste(outdir,"Rowmeans_of_ATAC_overHSFwithchip.svg", sep=""))
ggplot(data=meandflong, aes(x=position, y=depth,colour=interaction(genotype, tempature), linetype=interaction(genotype, tempature))) +
  geom_line(size=2)+scale_linetype_manual(values=c("solid","solid", "dashed","dashed"))+scale_color_manual(values=c('#006400', '#4B0082','#32CD32', '#DA70D6'))+theme(text = element_text(size=20))+ theme_minimal()+ theme(text = element_text(size = 20))
dev.off()

rowmeansfig <- ggplot(data=meandflong, aes(x=position, y=depth,colour=interaction(genotype, tempature), linetype=interaction(genotype, tempature))) +
    geom_line(size=2)+scale_linetype_manual(values=c("solid","solid", "dashed","dashed"))+scale_color_manual(values=c('#006400', '#4B0082','#32CD32', '#DA70D6'))+theme(text = element_text(size=20))+ theme_minimal()+ theme(text = element_text(size = 20))
#ggsave(file=paste(outdir,"Rowmeans_of_ATAC_overHSFwithchip.svg", sep=""), plot=rowmeansfig)

svg(paste(outdir,"EnrichedHeatmap_of_ATAC_overHSFwithchip_diff_and_total.svg", sep=""))
draw(ht_list+ht_list_wtih_diffs)
dev.off()



png(paste(outdir,"EnrichedHeatmap_of_ATAC_overHSFwithchip_total_and_diff.png", sep=""), width = 1000, height = 500)
draw(ht_list+ht_list_wtih_diffs)
dev.off()
