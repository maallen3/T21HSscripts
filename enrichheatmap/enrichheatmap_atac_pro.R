...
motiffile = "/scratch/Shares/dowell/motifs/HOCOMOCO_HUMAN_v11_p1e-5_grch38/HSF1_HUMAN.H11MO.0.A.bed"
...
col_fun_atac = colorRamp2(c(0, maxquantatac), c("white", "blue"))

prominabsolute=0
promaxabsolute=0.5
protopano <- HeatmapAnnotation(enriched = anno_enriched(ylim = c(prominabsolute, promaxabsolute)))
...
if (promaxabsolute==0){
  ...
  p<-EnrichedHeatmap(mat0pos, col = col_fun_fwd, name=paste(samplename, "PRO+", sep=""), column_title = paste(samplename," +"), top_annotation = protopano)
  n<-EnrichedHeatmap(mat0neg, col = col_fun_rev, name=paste(samplename, "PRO-", sep=""), column_title = paste(samplename," -"), top_annotation = protopano)
}
...
for (i in 2:length(prosamplenames)){
  ...
  if (promaxabsolute==0){
    ...
    p<-EnrichedHeatmap(mat0pos, col = col_fun_fwd, name=paste(samplename, "PRO+", sep=""), column_title = paste(samplename," +"), top_annotation = protopano,show_heatmap_legend = FALSE)
    n<-EnrichedHeatmap(mat0neg, col = col_fun_rev, name=paste(samplename, "PRO-", sep=""), column_title = paste(samplename," -"), top_annotation = protopano,show_heatmap_legend = FALSE)}
...
maxdiff_pro=0.5
prodifftopano <- HeatmapAnnotation(enriched = anno_enriched(ylim = c(mindiff_pro, maxdiff_pro)))
...
if (maxdiff_pro==0){
  ...
  T21diffpos <- EnrichedHeatmap(T21diffposmatrix, name="diff T21 pos", col = col_fun_pro_diff,  column_title = "diff T21 pos", top_annotation = prodifftopano)
  D21diffpos <- EnrichedHeatmap(D21diffposmatrix, name="diff D21 pos", col = col_fun_pro_diff, column_title = "diff D21 pos", top_annotation = prodifftopano,show_heatmap_legend = FALSE)
  T21diffneg <- EnrichedHeatmap(T21diffnegmatrix, name="diff T21 neg", col = col_fun_pro_diff,  column_title = "diff T21 neg", top_annotation = prodifftopano,show_heatmap_legend = FALSE)
  D21diffneg <- EnrichedHeatmap(D21diffnegmatrix, name="diff D21 neg", col = col_fun_pro_diff, column_title = "diff D21 neg", top_annotation = prodifftopano,show_heatmap_legend = FALSE)}
...
atacminabsolute=0
atacmaxabsolute=16
atactopano <- HeatmapAnnotation(enriched = anno_enriched(ylim = c(atacminabsolute, atacmaxabsolute)))
...
if (atacmaxabsolute==0){
  ...
  b<-EnrichedHeatmap(mat, col = col_fun_atac, name=paste(samplename, "ATAC", sep=""), column_title = paste(samplename," +"), top_annotation = atactopano)}
ht_list_atac <- b
...
for (i in 2:length(atacsamplenames)){
  ...
  if (atacmaxabsolute==0){
    ...
    b<-EnrichedHeatmap(mat, col = col_fun_atac, name=paste(samplename, " ATAC", sep=""), column_title = paste(samplename," +"), top_annotation = atactopano,show_heatmap_legend = FALSE)}
...
#if you don't know what to set maxdiff at use 0 and it will autoset but it will not be the same for all samples diffs
maxdiff_atac=4
atacdiffano <- HeatmapAnnotation(enriched = anno_enriched(ylim = c(mindiff_atac, maxdiff_atac)))
if (maxdiff_atac==0){
  ...
  T21diff <- EnrichedHeatmap(T21diffmatrix, name="diff T21", col = col_fun_atac_diff,  column_title = "diff T21", top_annotation = atacdiffano)
  D21diff <- EnrichedHeatmap(D21diffmatrix, name="diff D21", col = col_fun_atac_diff, column_title = "diff D21", top_annotation = atacdiffano,show_heatmap_legend = FALSE)
  }
...
ht2 <- T21diff+D21diff+ht_list_atac+ht_list_wtih_diffs+ht_list
